...
trait SimpleSynthesizer extends Synthesizer {
  ...
  private def getCoveredFrom(node: SynNode): Map[RhsNode, Vector[Ast]] = {
    ...
    def aux(
      ...
      val (ss, nts) =
        rhs.symbols.flatMap(s => s.getNt.map((s, _))).toVector.unzip
      val opts =
        (for { case (_: Optional, i) <- ss.zipWithIndex } yield i).toSet
      ...
      val minChildren =
        for { ((_, i), child) <- nts.zipWithIndex zip children } yield
