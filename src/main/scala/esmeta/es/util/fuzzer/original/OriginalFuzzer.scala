package esmeta.es.util.fuzzer.original
...
class OriginalFuzzer(
  ...
  /** mutator */
  given CFG = cfg
  val mutator: OriginalMutator = OriginalWeightedMutator(
    OriginalNearestMutator() -> 6,
    OriginalRandomMutator() -> 3,
    OriginalStatementInserter() -> 1,
    OriginalRemover() -> 1,
    OriginalSpecStringMutator() -> 1,
  )
  ...
  val initPool = init
    ...
    .getOrElse(
      OriginalSimpleSynthesizer(grammar).initPool
        .map(OriginalSimpleSynthesizer(grammar).name -> _) ++
      OriginalBuiltinSynthesizer(cfg.spec.algorithms).initPool
        .map(OriginalBuiltinSynthesizer(cfg.spec.algorithms).name -> _),
    )
